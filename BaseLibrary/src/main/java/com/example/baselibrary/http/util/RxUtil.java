...
public class RxUtil {
    ...
    public static void onMain(Action action) {
        Observable.create(e -> action.run()).subscribeOn(AndroidSchedulers.mainThread()).subscribe();
    }
    ...
    public static void delay(Action action, int time, RxAppCompatActivity activity) {
        Observable.create(e -> e.onNext("")).delay(time, TimeUnit.MILLISECONDS)
